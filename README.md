...
1) [질환명 검색시 추천](#질환명-검색시-추천)  
2) [검색어 없을 시 '검색어 없음'](#검색어-없을-시-'검색어-없음')   
3) [API 호출 최적화](#API-호출-최적화)
4) [키보드로 추천 검색어 이동](#키보드로-추천-검색어-이동) 
5) [도전 과제: 배포](#도전-과제:-배포) 
6) [도전 과제: 퍼지 문자열 검색](#도전-과제:-퍼지-문자열-검색) 
7) [도전 과제: 사용자가 입력한 질환명 볼드처리](#도전-과제:-사용자가 입력한-질환명-볼드처리) 
8) [추가 작업: Pagenation](#추가-작업:-Pagenation)  
9) [추가 작업: 다크모드](#추가-작업:-다크모드) 
10) [추가 작업: 검색결과 클릭시 실제 검색 연결](#추가-작업:-검색결과-클릭시-실제-검색 연결)
